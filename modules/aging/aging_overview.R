...
aging_overview_ui <- function(id) {
  ...
  fluidRow(
           box(
             ...
             fluidRow(
               column(width = 6,
                      highcharter::highchartOutput(NS(id, "pop_proj_all_aging"))),
               column(width = 6,
             ...
             fluidRow(
               column(width = 12,
                      h2("Key Takeaways"),
                      includeMarkdown("markdown/aging/overview/overview_key_takeaways.md"))
             )
             )
  )
...
aging_overview_server <- function(id, df) {
  ...
  output$pop_proj_all_aging <- highcharter::renderHighchart({
  ...
  outputOptions(output, "pop_proj_all_aging", suspendWhenHidden = FALSE)
  
  outputOptions(output, "pop_proj_65_by_race", suspendWhenHidden = FALSE)
