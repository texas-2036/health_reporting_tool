...
states_names <- c('California', 'Colorado', 'Florida', 'Georgia', 'Illinois', 'New York', 'North Carolina', 'Ohio', 'Pennsylvania', 'Virginia', 'Washington')

state_short <- c('CA', 'CO', 'FL', 'GA', 'IL', 'NY', 'NC', 'OH', 'PA', 'VA', 'WA')

peer <- c('Y', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y', 'Y')

states <- data.frame(states_names, state_short, peer)
...
## Data Source: https://demographics.texas.gov/Data/TPEPP/Projections/
...
## Data Source: America's Health Rankings
...
social_iso <- read_csv("raw_data/Aging/2020-Senior.csv") %>%
  ...
  full_join(y=states, by = c('state_name' = 'states_names'), copy= FALSE) %>%
...
alh_data <- read_csv("raw_data/Aging/Data Table.csv")%>%
  clean_names() %>%
  full_join(y=states, by = c('state' = 'state_short'), copy= FALSE) %>%
...
## Data Source: https://stateofchildhoodobesity.org/wic/
...
wic_obes <- read_csv('raw_data/Children/ExportCSV.csv') %>%
  clean_names() %>%
  full_join(y=states, by = c('location_desc' = 'states_names'), copy= FALSE) %>%
...
## Data Source: https://yrbs-explorer.services.cdc.gov/#/
...
## source for current use of electronic vapor products:  https://yrbs-explorer.services.cdc.gov/#/tables?questionCode=H35&topicCode=C02&location=TX&year=2019

#this is electronic vape usage
child_vape_use <- read_csv('raw_data/Children/current_vape_use.csv') %>%
  clean_names() %>%
  full_join(states, by = c('location_desc' = 'states_names'), copy=FALSE) %>%
  write_rds('clean_data/Children/child_vape_data.rds')

## source for current any tobacco use. https://yrbs-explorer.services.cdc.gov/#/tables?questionCode=QNTB4&topicCode=C02&location=XX&year=2019
...
##source for current smoking. : https://yrbs-explorer.services.cdc.gov/#/graphs?questionCode=QNTB4&topicCode=C02&location=XX&year=2019 
...
## Data Source: https://www.jpeds.com/article/S0022-3476(18)31292-7/fulltext 
...
## Data Source: https://www.census.gov/data-tools/demo/sahie/#/


child_inse <- read_csv('raw_data/Children/SAHIE_28SEP20_15_39_23_13.csv') %>%
  clean_names() %>%
  full_join(y=states, by = c('name' = 'states_names'), copy= FALSE) %>%
...
## Data Source: https://nccd.cdc.gov/dnpao_dtm/rdPage.aspx?rdReport=DNPAO_DTM.ExploreByLocation&rdRequestForwarding=Form

#Texas data

working_act_tx <-read_csv('raw_data/Working Age/ExportCSV_TX.csv') %>%
  clean_names() %>%
  write_rds('clean_data/Working Age/working_activity_tx.rds')

# US Data
working_act_us <-read_csv('raw_data/Working Age/ExportCSV_US.csv') %>%
  clean_names() %>%
  write_rds('clean_data/Working Age/working_activity_us.rds')
```
...
## Data Source: https://nccd.cdc.gov/DHDSPAtlas/Default.aspx?state=TX
